...
},
{
  ...
  const handlePayment = async(plan) => {
    try{
      ...
      plan.id === "basic" ? 100 :
      plan.id === "pro" ? 500 : 0;
      const result = await axios.post(ServerApi+"/api/payment/order", {
        planId:plan.id,
        amount:amount,
      ...
      const options = {
        ...
        handler:async function(response){
          const verifypay = await axios.post(ServerApi+"/api/payment/verify",response,{withCredentials:true})
    ...
    <div className='min-h-screen bg-gradient-to-br from-gray-50 to-emerald-5 py-16 px-6'>
      ...
      <div className="text-center w-full">
        ...
        <p className='text-gray-500 mt-3 text-lg'>
          Flexible pricing to match your preparation goals
        </p>
      </div>
      </div>
